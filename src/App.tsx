...
const RecipeTags = ({ onTagClick }) => {
  ...
  if (loading) {
    return <p>Loading tags...</p>;  // Show loading message
  }
...
const RecipeList = ({ tag, onBackClick }) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://dummyjson.com/recipes/tag/${tag}`)  // Use backticks for template literals
      .then((response) => response.json())
      .then((data) => {
        setRecipes(data.recipes);  // Assuming the response has a 'recipes' field
        setLoading(false);  // Stop loading when data is fetched
      })
      .catch((error) => {
